...
def run_qa_alignment_agent(
    ...
    streamlit_smoke_result = next(
        (
            check
            for check in input_model.run_test_and_fix_output.check_results
            if check.check_name == "streamlit_smoke"
        ),
        None,
    )
    streamlit_smoke_ran = streamlit_smoke_result is not None
    streamlit_smoke_passed = bool(streamlit_smoke_result and streamlit_smoke_result.passed)

    if not quiz_type_balance_ok:
        issues.append("Quiz type distribution is not 4 types × 2 items.")
    ...
    return QAAlignmentOutput(
        ...
        qa_checklist=[
            ...
            f"4개 quiz_type × 각 2문항 여부: {'PASS' if quiz_type_balance_ok else 'FAIL'}",
        ...
        change_log_entries=[
            ...
            (
                ...
                f"8문항={item_count == 8}, "
                f"4유형x2문항={quiz_type_balance_ok}, "
