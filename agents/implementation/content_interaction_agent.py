...
ALLOWED_LEARNING_DIMENSIONS = ["구체성", "맥락성", "목적성", "종합성"]
...
def run_content_interaction_agent(
    ...
    summary = _repair_and_validate_content(output, input_model, llm_client)
    output.semantic_validation = summary
    _synchronize_output_maps(output)
...
def _repair_and_validate_content(
    ...
    initial_assessments = [_assess_item(item) for item in output.items]

    for item, assessment in zip(output.items, initial_assessments):
        _apply_allowed_label_corrections(item, assessment)

    regeneration_plan = _plan_regeneration(output.items)
    regenerated_item_ids: list[str] = []

    if regeneration_plan:
        for item_index, target_quiz_type in regeneration_plan:
            original_item = output.items[item_index]
            target_dimension = _assess_item(original_item).expected_learning_dimension
        ...
        final_assessments = [_assess_item(item) for item in output.items]
    ...
    learning_dimension_values_valid = all(
        item.learning_dimension in ALLOWED_LEARNING_DIMENSIONS for item in output.items
    )
    quiz_type_distribution_valid = all(
        quiz_type_counts.get(quiz_type) == 2 for quiz_type in CANONICAL_QUIZ_TYPES
    )
    semantic_validator_passed = (
        learning_dimension_values_valid
    ...
    if not semantic_validator_passed:
        raise ValueError(_build_validation_failure_message(final_assessments, quiz_type_counts))
...
def _synchronize_output_maps(output: ContentInteractionOutput) -> None:
...
def _plan_regeneration(items: list[QuizItem]) -> list[tuple[int, str]]:
    assessments = [_assess_item(item) for item in items]
    regeneration_targets: dict[int, str] = {
        index: assessment.expected_quiz_type
        for index, assessment in enumerate(assessments)
        if assessment.requires_regeneration
    }

    typed_indices: dict[str, list[int]] = {quiz_type: [] for quiz_type in CANONICAL_QUIZ_TYPES}
    for index, item in enumerate(items):
        typed_indices[item.quiz_type].append(index)

    deficits: list[str] = []
    extras: list[int] = []
    for quiz_type in CANONICAL_QUIZ_TYPES:
        count = len(typed_indices[quiz_type])
        if count < 2:
            deficits.extend([quiz_type] * (2 - count))
        elif count > 2:
            extras.extend(typed_indices[quiz_type][2:])

    for item_index, target_quiz_type in zip(extras, deficits):
        regeneration_targets[item_index] = target_quiz_type

    return sorted(regeneration_targets.items(), key=lambda item: item[0])
...
def _assess_item(item: QuizItem) -> ItemAssessment:
    expected_quiz_type, quiz_type_reasons = _infer_expected_quiz_type(item)
    expected_learning_dimension, learning_reasons = _infer_expected_learning_dimension(item)
    reasons = quiz_type_reasons + learning_reasons
    requires_regeneration = False

    if expected_quiz_type is None:
        expected_quiz_type = item.quiz_type
        reasons.append("문항 행동을 4개 상호작용 유형 중 하나로 안정적으로 분류할 수 없습니다.")
        requires_regeneration = True

    if expected_learning_dimension is None:
        expected_learning_dimension = item.learning_dimension
        reasons.append("문항이 강화하는 질문력 차원을 안정적으로 판정할 수 없습니다.")
        requires_regeneration = True

    if _has_action_shape_mismatch(item, expected_quiz_type):
...
def _infer_expected_learning_dimension(item: QuizItem) -> tuple[str | None, list[str]]:
    question_text = item.question
...
def _validate_content_contract(output: ContentInteractionOutput) -> None:
    ...
    for item in output.items:
        ...
        if item.learning_dimension not in ALLOWED_LEARNING_DIMENSIONS:
            raise ValueError(
